# equalizar a imagem a cores
# converter BGR em HSV
# equalizar o canal V
# converter a imagem equalizada para BGR
# mostrar em RGB
...
for i, file in enumerate(files):
    ...
    pdf = ivc_gray_pdf(image_hsv[:,:,2])
    cdf = ivc_pdf_2_cdf(pdf)
    g = ivc_equalize_gray(image_hsv[:,:,2], cdf)
    pdf_g = ivc_gray_pdf(g)
    cdf_g = ivc_pdf_2_cdf(pdf_g)
    image_hsv_equalized = image_hsv.copy()
    image_hsv_equalized[:,:,2] = g
    # image_hsv_equalized = image_hsv[:,:,0:2]
    # image_hsv_equalized = np.concatenate(image_hsv_equalized, axis=2)
    image_equalized = cv2.cvtColor(image_hsv_equalized, cv2.COLOR_HSV2BGR)

    plt.subplot(4, 8, i*8+1)
    plt.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
    plt.title("image")
    plt.xticks([])
    plt.yticks([])
    plt.subplot(4, 8, i*8+2)

    plt.imshow(cv2.cvtColor(image_hsv[:,:,2], cv2.COLOR_GRAY2RGB))
    plt.title("V")
    plt.xticks([])
    plt.yticks([])

    plt.subplot(4, 8, i*8+3)
    plt.bar(range(256), pdf)
    plt.title("pdf")

    plt.subplot(4, 8, i*8+4)
    plt.plot(cdf)
    plt.title("cdf")

    plt.subplot(4, 8, i*8+5)
    plt.imshow(cv2.cvtColor(g, cv2.COLOR_GRAY2RGB))
    plt.title("V Equalized")
    plt.xticks([])
    plt.yticks([])

    plt.subplot(4, 8, i*8+6)
    plt.bar(range(256), pdf_g)
    plt.title("pdf_g")

    plt.subplot(4, 8, i*8+7)
    plt.plot(cdf_g)
    plt.title("cdf_g")

    plt.subplot(4, 8, i*8+8)
